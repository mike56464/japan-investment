...
print(f"📊 第一銀行專用 - 系統測試版 (v5.1 強制報錯檢測) | {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("-" * 50)
...
if jpy_rate and rate_date:
    ...
    if supabase:
        try:
            ...
            # 【重要修改】：加上 on_conflict 參數或改用 insert 來捕捉真實錯誤
            # 如果發生錯誤，這裡會印出詳細的 API 阻擋原因
            try:
                # 先嘗試最基本的 insert
                ...
                if "duplicate key" in str(inner_e).lower() or "23505" in str(inner_e):
                    res = supabase.table("exchange_rates").upsert(rate_payload,
                                                                  on_conflict="currency_code,rate_date").execute()
